...
import { IconTransition } from '@/components/ui/tranisition/IconTransition';
...
export const PinIconToggle: Component<{
  ...
  return (
    <MotionButtonBase
      ...
      className={clsxm(
        'absolute top-[4px] right-0 bottom-0 z-10 -m-5 box-content hidden size-5 items-center p-5',
        isLogged &&
          'inline-flex cursor-pointer opacity-50 transition-opacity hover:opacity-100',
        !isLogged && pin && 'pointer-events-none',
        pin && 'text-uk-red-light !inline-flex opacity-100',
        className,
      )}
      onClick={handlePin}
    >
